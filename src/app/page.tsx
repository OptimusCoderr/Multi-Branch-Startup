...
function DashboardMockup() {
  return (
    <div className="w-full max-w-md overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-2xl shadow-violet-200/40 dark:border-gray-800 dark:bg-gray-900 dark:shadow-black/40 sm:max-w-lg">
      <div className="flex items-center gap-1.5 border-b border-gray-100 px-4 py-3 dark:border-gray-800">
        ...
        <span className="h-2.5 w-2.5 rounded-full bg-gray-200 dark:bg-gray-700" />
        <span className="h-2.5 w-2.5 rounded-full bg-gray-200 dark:bg-gray-700" />
        <span className="ml-3 flex items-center gap-1.5 text-[11px] font-medium text-gray-400 dark:text-gray-500">
          <LogoPlaceholder size={13} color="#7c3aed" />
          app.multibranchinventory.com/dashboard
        </span>
      </div>

      <div className="flex items-center gap-4 border-b border-gray-100 px-4 py-2.5 text-[11px] font-medium text-gray-400 dark:border-gray-800 dark:text-gray-500">
        <span className="rounded-md bg-violet-50 px-2 py-1 text-violet-700 dark:bg-violet-950 dark:text-violet-300">Dashboard</span>
        <span>Products</span>
        <span>Sales</span>
        <span>Customers</span>
      ...
      <div className="flex flex-col gap-4 p-4 sm:p-5">
        <div>
          <p className="font-display text-sm font-semibold text-gray-900 dark:text-gray-100">Business overview</p>
          <p className="text-[11px] text-gray-400 dark:text-gray-500">Today across every branch</p>
        </div>
        ...
        <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-4">
          {[
            ...
            <div key={stat.label} className="rounded-lg border border-gray-100 bg-gray-50/70 p-2.5 dark:border-gray-800 dark:bg-gray-800/40">
              <div
              ...
              <p className="text-[9px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">{stat.label}</p>
        ...
        <div className="rounded-lg border border-gray-100 bg-gray-50/70 p-3 dark:border-gray-800 dark:bg-gray-800/40">
          <div className="mb-2 flex items-center justify-between">
            <p className="text-[10px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">Sales, last 12 weeks</p>
...
function PhoneMockup() {
  return (
    <div className="w-[184px] rounded-[1.75rem] border-[6px] border-gray-900 bg-gray-900 shadow-2xl shadow-black/30 sm:w-[204px]">
      <div className="overflow-hidden rounded-[1.35rem] bg-white dark:bg-gray-950">
        <div className="flex items-center justify-between px-3 pt-2.5 text-[9px] font-medium text-gray-400 dark:text-gray-500">
          <span>9:41</span>
          <span className="h-1.5 w-8 rounded-full bg-gray-200 dark:bg-gray-800" />
        </div>
        ...
        <div className="p-3">
          ...
          <div className="mt-3 grid grid-cols-2 gap-2">
            {[
              ...
              <div key={stat.label} className="rounded-lg border border-gray-100 bg-gray-50/70 p-2 dark:border-gray-800 dark:bg-gray-900/60">
                <div
                ...
                <p className="text-[7px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">{stat.label}</p>
                <p className="text-[11px] font-semibold text-gray-900 dark:text-gray-100">{stat.value}</p>
              </div>
            ))}
          </div>

          <div className="mt-3 flex items-center gap-1.5 text-[9px] font-semibold uppercase tracking-wide text-gray-400 dark:text-gray-500">
          ...
          <div className="mt-1.5 flex flex-col gap-1.5">
            {["Low stock — 3 products", "2 invoices unpaid"].map((item) => (
              <div
                key={item}
                className="rounded-lg border border-gray-100 bg-gray-50/70 px-2 py-1.5 text-[9px] text-gray-600 dark:border-gray-800 dark:bg-gray-900/60 dark:text-gray-400"
              >
                {item}
              </div>
            ))}
  ...
  return (
    <div className="w-full max-w-sm overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl shadow-violet-200/30 dark:border-gray-800 dark:bg-gray-900 dark:shadow-black/40">
      <div className="flex items-center justify-between border-b border-gray-100 px-4 py-3 dark:border-gray-800">
        <p className="text-xs font-semibold text-gray-900 dark:text-gray-100">New sale</p>
        <span className="flex items-center gap-1 rounded-md bg-violet-50 px-2 py-1 text-[10px] font-semibold text-violet-700 dark:bg-violet-950 dark:text-violet-300">
      ...
      <div className="flex flex-col gap-2 p-4">
        {lines.map((line) => (
          <div
            key={line.name}
            className="flex items-center justify-between rounded-lg border border-gray-100 bg-gray-50/70 px-3 py-2 dark:border-gray-800 dark:bg-gray-800/40"
          >
            <div>
              <p className="text-xs font-medium text-gray-800 dark:text-gray-200">{line.name}</p>
              <p className="text-[10px] text-gray-400 dark:text-gray-500">Qty {line.qty}</p>
            </div>
            <p className="text-xs font-semibold text-gray-900 dark:text-gray-100">{line.price}</p>
          </div>
        ))}
        <div className="mt-1 flex items-center justify-between border-t border-gray-100 pt-3 dark:border-gray-800">
          ...
          <p className="text-sm font-bold text-gray-900 dark:text-gray-100">₦66,500</p>
        </div>
...
function TransferMockup() {
  ...
  const tones = {
    brand: "bg-violet-50 text-violet-700 dark:bg-violet-950 dark:text-violet-300",
    ...
    <div className="w-full max-w-sm overflow-hidden rounded-2xl border border-gray-200 bg-white shadow-xl shadow-violet-200/30 dark:border-gray-800 dark:bg-gray-900 dark:shadow-black/40">
      ...
      <div className="flex flex-col gap-2 p-4">
        {rows.map((row) => (
          ...
          >
            <div>
              <p className="text-xs font-medium text-gray-800 dark:text-gray-200">{row.product}</p>
              <p className="text-[10px] text-gray-400 dark:text-gray-500">{row.route}</p>
            </div>
    ...
    <div className={`rounded-3xl p-8 sm:p-12 ${bg}`}>
      <div className={`mx-auto flex max-w-5xl flex-col items-center gap-10 lg:flex-row ${reverse ? "lg:flex-row-reverse" : ""}`}>
        <div className="flex-1 text-center lg:text-left">
          ...
          <h3 className="mt-2 font-display text-2xl font-semibold tracking-tight text-gray-900 dark:text-gray-100 sm:text-3xl">{title}</h3>
          <p className="mt-3 max-w-md text-gray-500 dark:text-gray-400 lg:max-w-none">{body}</p>
          <Link
            href="/sign-up"
            className="mt-5 inline-flex items-center gap-1.5 text-sm font-semibold text-violet-700 hover:underline dark:text-violet-400"
          >
    ...
    <main className="flex min-h-screen flex-col">
      <div className="relative flex flex-col overflow-hidden px-4 pb-16 pt-20 text-center sm:pt-28">
        ...
        <div className="mx-auto mb-6 flex items-center gap-2.5">
          ...
          <span className="font-display text-base font-semibold text-gray-900 dark:text-gray-100">Multi-Branch Inventory</span>
        </div>

        <h1 className="mx-auto max-w-2xl font-display text-4xl font-semibold tracking-tight text-gray-900 dark:text-gray-100 sm:text-5xl">
          Run every branch{" "}
          <span className="bg-clip-text text-transparent" style={{ backgroundImage: "linear-gradient(135deg, #7c3aed, #1e1b2e)" }}>
            from one place
          </span>
        </h1>
        <p className="mx-auto mt-5 max-w-md text-gray-500 dark:text-gray-400">
        ...
        <div className="mx-auto mt-8 flex flex-wrap justify-center gap-3">
          ...
          <Link
            href="/sign-in"
            className="rounded-lg border border-gray-300 px-5 py-2.5 text-sm font-semibold text-gray-700 transition-colors hover:bg-gray-50 dark:border-gray-700 dark:text-gray-200 dark:hover:bg-gray-900"
          >
            Sign in
          </Link>
        </div>
        ...
        <div className="relative mt-14 -mx-4 overflow-x-auto px-4">
          <div className="mx-auto flex w-max gap-2.5 sm:justify-center">
            {PILLS.map((pill) => (
              <span
                key={pill}
                className="whitespace-nowrap rounded-full border border-gray-200 bg-white px-3.5 py-1.5 text-xs font-medium text-gray-600 shadow-sm dark:border-gray-800 dark:bg-gray-900 dark:text-gray-300"
              >
                {pill}
              </span>
            ))}
      ...
      <div className="relative overflow-hidden bg-gradient-to-b from-violet-50/70 to-white px-4 py-20 dark:from-violet-950/20 dark:to-gray-950 sm:py-28">
        ...
        <div className="relative mx-auto hidden h-[560px] max-w-4xl sm:block">
          {CLOUD_FEATURES.map((f, i) => (
            ...
            >
              <div className="flex h-14 w-14 items-center justify-center rounded-full bg-white text-violet-600 shadow-md shadow-violet-200/60 dark:bg-gray-900 dark:text-violet-400 dark:shadow-black/40">
        ...
        <div className="mx-auto mt-10 grid max-w-md grid-cols-3 gap-4 sm:hidden">
          {CLOUD_FEATURES.map((f) => (
            <div key={f.label} className="flex flex-col items-center gap-1.5 text-center">
              <div className="flex h-11 w-11 items-center justify-center rounded-full bg-white text-violet-600 shadow-sm dark:bg-gray-900 dark:text-violet-400">
                <f.icon size={18} strokeWidth={2} />
              </div>
              <p className="text-[11px] font-medium text-gray-600 dark:text-gray-400">{f.label}</p>
            </div>
          ))}
        </div>
      </div>

      <div className="border-t border-gray-100 bg-gray-50 px-4 py-16 dark:border-gray-900 dark:bg-gray-950">
        <div className="mx-auto grid max-w-4xl grid-cols-1 gap-6 sm:grid-cols-2">
          {FEATURES.map((f) => (
            <div
              key={f.title}
              className="flex gap-4 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm dark:border-gray-800 dark:bg-gray-900"
            >
              ...
              <div>
                <p className="font-display font-semibold text-gray-900 dark:text-gray-100">{f.title}</p>
                <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">{f.body}</p>
              </div>
            </div>
          ))}
        </div>

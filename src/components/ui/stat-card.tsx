  ...
  const content = (
    <>
      ...
      <div>
        ...
        <p className="mt-1 font-display text-2xl font-semibold text-gray-900 dark:text-gray-100">{value}</p>
        {detail && <p className="mt-0.5 text-xs text-gray-500 dark:text-gray-400">{detail}</p>}
      </div>
    </>
  );
  const className =
    "flex flex-col gap-3 rounded-2xl border border-gray-200 bg-white p-5 shadow-sm transition-shadow hover:shadow-md dark:border-gray-800 dark:bg-gray-900";

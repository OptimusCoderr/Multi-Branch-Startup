...
export function AppShell({
  ...
  return (
    <div className="flex min-h-screen">
      ...
      <aside
        className={`fixed inset-y-0 left-0 z-40 flex w-64 shrink-0 -translate-x-full flex-col border-r border-gray-200 bg-white transition-transform duration-200 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0 print:hidden ${
          mobileOpen ? "translate-x-0" : ""
        }`}
      >
        <div className={`relative flex items-center gap-2.5 border-b border-gray-100 px-5 ${compact ? "py-3" : "py-5"}`}>
          {logoUrl ? (
          ...
          <div className="min-w-0 flex-1">
            <p className="truncate font-display text-sm font-semibold leading-tight">{companyName}</p>
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <div className="sticky top-0 z-20 flex items-center gap-3 border-b border-gray-100 bg-white/90 px-4 py-3 backdrop-blur lg:hidden">
          <button
            type="button"

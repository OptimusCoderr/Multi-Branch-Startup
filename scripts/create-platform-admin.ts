/**
 * Creates (or promotes) a platform-admin account — a person who can see
 * every company via /admin, entirely outside the per-company Membership/
 * Role system. Run manually, once per environment:
 *
 ...
 * alone and only the isPlatformAdmin flag is set — safe to re-run.
 */
...
async function main() {
  ...
  if (existing) {
    ...
    console.log(`Existing account ${email} promoted to platform admin. Password unchanged.`);
    return;
  }

  const password = generateStrongPassword();
  const result = await auth.api.signUpEmail({ body: { email, name, password } });
  await prisma.user.update({ where: { id: result.user.id }, data: { isPlatformAdmin: true } });
